...
async function testBelowGridBuysAreDeferredNotAdoptedOrCancelled() {
    ...
    const chain = [
        makeChainOrder('1.7.574140065', ORDER_TYPES.BUY, LEVELS[0], 5),
        makeChainOrder('1.7.574140066', ORDER_TYPES.BUY, 85, 5),
        makeChainOrder('1.7.574140067', ORDER_TYPES.BUY, 80, 5),
    ];
    ...
    for (const id of ['1.7.574140066', '1.7.574140067']) {
    ...
    assert.strictEqual(slot0.orderId, '1.7.574140065', 'slot-0 must keep its live order (no mis-adoption)');
...
async function testBelowGridBuyDoesNotAdoptIntoFreeSlot0() {
    ...
    const result = await engine.syncFromOpenOrders(
        [makeChainOrder('1.7.574140068', ORDER_TYPES.BUY, 82, 5)],
        { skipAccounting: true }
    );
...
async function testAboveGridSellIsDeferredNotCancelled() {
    ...
    const result = await engine.syncFromOpenOrders(
        [
            makeChainOrder('1.7.574140070', ORDER_TYPES.SELL, LEVELS[topIdx], 10),
            makeChainOrder('1.7.574140071', ORDER_TYPES.SELL, LEVELS[topIdx] * 1.2, 10),
        ],
        { skipAccounting: true }
    );
    ...
    assert.strictEqual(correctionsFor(mgr, '1.7.574140071').length, 0, 'Above-grid orphan must NOT be queued for cancellation');
    const top = mgr.orders.get(`slot-${topIdx}`);
    assert.strictEqual(top.orderId, '1.7.574140070', 'Top slot must keep its live order');
...
async function testInRailOrphanStillAdopts() {
    ...
    const engine = new SyncEngine(mgr);
    const result = await engine.syncFromOpenOrders(
        [makeChainOrder('1.7.574140072', ORDER_TYPES.BUY, LEVELS[idx], 10)],
        { skipAccounting: true }
    );
    ...
    assert.strictEqual(slot.orderId, '1.7.574140072', 'In-rail slot must bind the chain order');
