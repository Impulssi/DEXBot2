...
| v1.4.5 → v1.4.6 | 3 | AsyncLock ALS re-entrancy fix, grid type reassignment on load, 8 single-value fields hardened to refcounts/stacks, sync engine ABBA deadlock fix via gridLockAlreadyHeld skip, lock hierarchy Level 0/1 doc sync, nested re-entrancy + ABBA deadlock tests |
