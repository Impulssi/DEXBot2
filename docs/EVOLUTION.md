...
| v1.4.5 → v1.4.6 | 8 | AsyncLock ALS re-entrancy fix (single symbol → Set), grid type reassignment on load (all slots, not just VIRTUAL), 8 single-value fields hardened to refcounts/stacks, lock hierarchy correction (swapped _syncLock/_gridLock levels, eliminated gridLockAlreadyHeld + 8 call sites, restructured grid_reconcile Phase 1/2), stale COW fund snapshot fix, gapSlots persistence for rebalance consistency, phantom order fund inflation fix, cross-chunk boundary shift cap, extract budget from config, loadGrid JSDoc side-effect documentation |
