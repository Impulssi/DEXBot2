...
- **Fix(ui)**: clarify the Grid Price label in the bot editor — the Price section displayed the gridPrice reference as bare "Grid:", which was ambiguous next to "Start:"/"Pool:" and collided with the section "4) Grid". Renamed to "Grid Price:" to match the config field name and the values it accepts (pool/book/ama/number/none) (`modules/account_bots.ts`).
