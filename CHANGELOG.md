...
- **Feat(editor)**: add Range quality legend with tiered coloring — `RANGE_QUALITY` thresholds (`GREEN_MIN 2.0` wide / `YELLOW_MIN 1.55` efficient / `ORANGE_MIN 1.45` tight / `RED_MAX 1.35` suicidal) drive a pre-entry legend and live `colorRangeValueByQuality` in `askNumberOrMultiplier`/`askMaxPrice` and summary Pair display (cyan) (`modules/constants.ts`, `modules/account_bots.ts`).
