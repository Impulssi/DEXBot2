...
- **Fix**: eliminate subscription health-check cycling and bootstrap fund-drift false positive — (1) server-side database API session TTL (~120s) silently killed notice stream while WebSocket stayed open; transport keepalive only pings `login_api` (API ID 1), not `database_api` (API ID 2). Fix: call `get_dynamic_global_properties()` at start of each health-check tick to keep database API session alive, eliminating repeating 3-min subscription death/resubscribe cycle (`modules/bitshares-native/subscriptions.ts`). (2) `initializeStartupState()` called `finishBootstrap()` in its `finally` block before grid was loaded into `manager.orders`; drift check saw `gridBuy = 0` vs locked chain balances, always reporting massive BUY-side drift. Fix: removed premature `finishBootstrap()` — now runs only after grid is fully loaded and reconciled (`modules/dexbot_startup_runtime.ts`).
...
- **Refactor**: address review concerns — deduplicate `_extractRateFromCollateralMap` helper in `credit_runtime.ts` eliminating 3 nearly-identical rate-extraction blocks; hoist `_resolveAsset` outside pricing loops so asset resolution runs once per refresh. Export `RUNTIME_SETTINGS_KEYS` from `runtime_settings.ts`; `dexbot_maintenance_runtime.ts` uses `getRuntimeSettingsKeys()` instead of hardcoded list. Reorder `finishBootstrap` after `fetchAccountTotals` in `dexbot_startup_runtime.ts` so bootstrap drift check uses fresh balances. Remove redundant inline DB ping from subscription health check (double-ping with `startSubscriptionKeepalive`). Add `Promise.race` timeout for `readOpenOrders` in `grid.ts` with proper `NodeJS.Timeout | undefined` typing. Derive `SYNC_LOCK_FORCE_RELEASE_AGE_MS` as `SYNC_LOCK_TIMEOUT_MS * 2` in `constants.ts` so user overrides propagate automatically (`modules/credit_runtime.ts`, `modules/runtime_settings.ts`, `modules/dexbot_maintenance_runtime.ts`, `modules/dexbot_startup_runtime.ts`, `modules/bitshares-native/subscriptions.ts`, `modules/order/grid.ts`, `modules/constants.ts`).
