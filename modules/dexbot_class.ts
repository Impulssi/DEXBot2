            ...
            if (!this.manager?._fillProcessingLock) {
                ...
                const startFinalFlush = (label: string): Promise<void> => {
                    ...
                    finalFlushPromise = (async () => {
                        ...
                        // Manual-cancel holds are deliberately NOT cleared
                        // here: they persist in the final snapshot, so every
                        // restart (graceful or crash) keeps operator-cancelled
                        // slots empty. Restore only via price-move expiry or
                        // `dexbot clear-holds <bot> [slot]`.
